// ==UserScript==
// @name         Gaia Online Friend List Search Across Pages
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  Search through all pages of friend
// @author       You
...
function addSearchBox() {
    ...
    searchBox.setAttribute("placeholder", "Search Friends...");
    ...
    function getAllPageLinks() {
        ...
        // Assuming the current page's URL follows the pattern: https://www.gaiaonline.com/profiles/{username}/{userid}/?mode=friends
        const baseUrl = window.location.href.split('?')[0];

        // URL for the first page
        const firstPageUrl = baseUrl + '?mode=friends';
...
async function searchUsers(searchTerm) {
    ...
    function setLoadingAnimation() {
        ...
        return setInterval(() => {
            ...
            searchBox.placeholder = `Searching Friend List${dotsText}`;
...
function displayResult(user, found, profileUrl) {
    ...
    if (found) {
        resultDiv.textContent = `${user} found on Friend List: `;
